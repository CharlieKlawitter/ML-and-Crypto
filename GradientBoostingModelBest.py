...
random_search = RandomizedSearchCV(GradientBoostingClassifier(), param_grid, n_iter=4, cv=kf, n_jobs=-1)
...
df_ml_results.loc[X_train.index, 'InSample_Probabilities'] = best_model.predict_proba(X_train).max(axis=1)  # Max probability for the predicted class
df_ml_results.loc[X_test.index, 'OutSample_Probabilities'] = best_model.predict_proba(X_test).max(axis=1)   # Max probability for the predicted class

df_ml_results.loc[X_train.index, 'InSample_Correct'] = (train_preds == y_train)  # True/False for in-sample
df_ml_results.loc[X_test.index, 'OutSample_Correct'] = (test_preds == y_test)    # True/False for out-of-sample
...
scores = cross_val_score(model, X, y, cv=kf, n_jobs=-1) #Added n_jobs to run cross validation concurrently
